...
import br.edu.utfpr.tsi.sd.core.web.dto.*;
...
public class SocketIoClient implements Client {
    ...
    public void sendControls(ControlsDto controlsDto) {
        var index = localStateSynchronizer.getCurrentIndex();
    ...
    private void setupEvents() {
        on(socket, Event.OTHER_PLAYER_CONNECTED, response -> {
            var gameStateDtoJson = (String) response[0];
        ...
        on(socket, Event.OTHER_PLAYER_DISCONNECTED, response -> {
            var playerIdJson = (String) response[0];
        ...
        on(socket, Event.GAME_STATE_SENT, response -> {
            var gameStateDtoJson = (String) response[0];
            var indexedDto = Dto.fromJsonString(gameStateDtoJson, IndexedGameStateDto.class);
